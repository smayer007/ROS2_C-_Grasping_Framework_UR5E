...
def generate_launch_description():
    ...
    moveit_simple_controllers_yaml = load_yaml(
        'mmp_moveit_config', 'config/moveit_controllers.yaml'
    )
    moveit_controllers = {
    ...
    # RViz
    rviz_base = os.path.join(get_package_share_directory('mmp_moveit_config'), 'config')
    rviz_full_config = os.path.join(rviz_base, 'belcamera.rviz')
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory("mmp_moveit_config"),
        "config",
    ...
    ur5e_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["arm_ur5e_controller","--controller-manager","/controller_manager"],
    )

    gripper_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["gripper_robotiq_controller", "--controller-manager", "/controller_manager"],
    )

  #Load controllers
    load_controllers = []
    for controller in ['joint_state_broadcaster','arm_ur5e_controller', 'gripper_robotiq_controller']:
        load_controllers += [
            ExecuteProcess(
    ...
    joint_state_publisher = Node(
        ...
        parameters=[
            {'source_list': ['ur5e/joint_states', 'robotiq_85_gripper/joint_states'], 'rate': 30}],

        #arguments=["joint_state_broadcaster"]
            #{'source_list': ['arm_ur5e_controller/joint_states', ' gripper_robotiq_controller/joint_states'], 'rate': 30}],
    )
